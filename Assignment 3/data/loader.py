...
class DataLoader(ABC):
    def __init__(self, path, verbose, seed, name):
        self._path = path
    ...
    def build_train_test_split(self, test_size=0.25):
...
class WineData(DataLoader):
    def __init__(self, path='data/winequality-red.csv', verbose=False, seed=1):
        # hydra changes the root dir when logging so we have to modify the path
        mod_path = path
        super().__init__(path, verbose, seed, name='redwine')
...
class DiabeticData(DataLoader):
    ...
    def _preprocess_data(self):
        ...
        self._data['Class'] = self._data['Class'].astype(int)
        return
