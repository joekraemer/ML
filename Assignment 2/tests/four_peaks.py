...
def construct_standard_four_peaks(complexity=40):
...
def exp_problem_complexity_with_parallel():
    complexity_list = range(10, 100, 20)

    num_cores = multiprocessing.cpu_count()
    inputs = tqdm(complexity_list)

    processed_list_all = Parallel(n_jobs=num_cores)(delayed(run_single_complexity)(i) for i in inputs)
    print("all complete")
...
def run_single_complexity(c):
    problem, init_state = construct_standard_four_peaks(c)
...
def exp_iterations():
    ...
    problem, init_state = construct_standard_four_peaks()
...
def run_four_peaks():
    ...
    hyperTester = HyperTester(construct_standard_four_peaks, 'four_peaks')
