...
class KColor(GenericTester):
    def __init__(self):
        hyper_tuning = {'ga_pop_size': [100, 300, 500],
                                 'mimic_default_keep': 0.3,
                                 'mimic_default_pop': 400,
                                 'mimic_pop_size': [100, 300, 500],
                                 'mimic_keep_percent': [0.15, 0.3, 0.5]}

        super().__init__(name='sixpeaks', complexity_list=range(10, 50, 20), hyper_config=hyper_tuning)

    def problem_constructor(self, complexity=40, seed=123456):
        max_connections = 3 # typically 1/5
        problem = SixPeaksGenerator().generate(seed=123456, number_of_nodes=complexity, max_connections_per_node=max_connections, maximize=True)
