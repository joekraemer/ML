...
class NNTests(BaseTest):
    def __init__(self, details):
        ...
        self._learner = NNLearner(alpha=self._details.NNL_alpha, n_nodes=self._details.NNL_n_nodes,
                                  n_layers=self._details.NNL_n_layers)
    ...
    def run_neurons_validation(self):
        ...
        plot_hyperparam_validation_curve(train_scores, test_scores, self.neurons_list, self.Name, 'Number of Nodes',
    ...
    def run_layers_validation(self):
        ...
        plot_hyperparam_validation_curve(train_scores, test_scores, self.layers_list, self.Name,
                                         'Number of Hidden Layers', folder=self._details.ds.name)
    ...
    def run_alpha_validation(self):
        ...
        plot_hyperparam_validation_curve(train_scores, test_scores, self.alpha_list, self.Name, 'alpha',
                                         folder=self._details.ds.name)
    ...
    def run_loss_curve(self):
        ...
        for _ in range(0, 9):
            temp_learner = NNLearner(alpha=self._details.NNL_alpha, n_nodes=self._details.NNL_n_nodes,
                                     n_layers=self._details.NNL_n_layers)
            res = cross_validate(
                temp_learner.Classifier,
                self._details.ds.train_x,
        ...
        plot_single_curve(loss_curves_df, 'NN_loss_curve', 'Loss Curve for Neural Network', 'Loss', 'iteration',
                          folder=self._details.ds.name)
